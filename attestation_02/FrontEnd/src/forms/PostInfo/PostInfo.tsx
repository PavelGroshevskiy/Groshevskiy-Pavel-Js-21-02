        ...
        <header className="post-info__header">
          ...
          <div className="post-info__user">
            <Popup comment={post.owner.id as string}>
              {`${post.owner.title && t(`title-items.${post.owner.title}`)} ${post.owner.lastName} ${
                post.owner.firstName
              }`}
            </Popup>
          </div>
          <div className="post-info__date">
            {t('date', { date: post.publishDate?.date, time: post.publishDate?.time })}
          </div>
        </header>
      )}
              ...
              {comments
                && comments.comments.map((value: CommentType) => (
                  <CommentCard
                    ...
                    date={value.publishDate || { date: '', time: '' }}
                    key={value.id}
                  />
                ))}
            </div>
