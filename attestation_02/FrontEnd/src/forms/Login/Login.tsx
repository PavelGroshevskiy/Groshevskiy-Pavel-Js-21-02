...
import {
  Spin, Button, Form, Input, Typography, Alert,
} from 'antd';
...
const Login = ({
  id, username, error, isLoading, signIn, putError,
}: Props) => {
      ...
      {!username && (
        <section className={`login ${themeContext.darkTheme && 'login_theme-dark'}`}>
          <header className="login__title">
            <Title level={3}>{t('login.signIn-header')}</Title>
          </header>
          ...
          <Form layout="vertical" labelCol={{ span: 24 }} wrapperCol={{ span: 24 }} form={form}>
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: t('login.rules.id-required'),
                },
                {
                  pattern: new RegExp(/^[a-z0-9]+$/i),
                  message: t('login.rules.id-allowed'),
                },
              ]}
            >
              <Input placeholder={t('login.id-placeholder')} />
            </Form.Item>
            <Form.Item>
