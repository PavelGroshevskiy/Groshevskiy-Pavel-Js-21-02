...
import {
  AUTH_ERROR, AUTH_LOG_OUT, AUTH_SIGN_IN, AUTH_SUCCESS,
...
const onSuccess = (draft: AuthState, user: UserType | undefined) => {
  ...
  if (user && user.id) {
    ...
    draft.username = `${user?.lastName}  ${user?.firstName}`;
  } else {
...
const authReducer = (state = initialState, action: AuthAction) => produce(state, (draft: AuthState) => {
  switch (action.type) {
    case AUTH_SIGN_IN:
      return signIn(draft);
    case AUTH_LOG_OUT:
